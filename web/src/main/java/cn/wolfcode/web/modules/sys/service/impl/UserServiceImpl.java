...
public class UserServiceImpl implements UserService {
    ...
    public List<String> getUserIds() {
        ...
        queryWrapper.lambda().select(SysUser::getUserId);

        //获取用户列表
        List<SysUser> userList = sysUserService.list(queryWrapper);
        //分离出用户id列表
        List<String> userIds = userList.stream() //创建流处理每个SysUser对象
                .map(SysUser::getUserId) //将每个SysUser对象映射为userId字段, 即使用getUserId()方法提取userId字段的值
                .collect(Collectors.toList()); //将流的元素收集到列表中
