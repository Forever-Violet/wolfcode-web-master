layui.use(['form', 'layer', 'table', 'laytpl', 'laydate', 'formSelects'], function () {
    ...
    var $ = layui.$, active = {
        reload: function () {
            //表格重载
            tableIns.reload({ //where 属性值传递给tableIns.reload()方法, 以便对表格数据进行过滤或筛选操作。
                where: buildCondition() // 调用buildCondition()获取where属性值
            });
        }
    };

    $('#userSearchBtn').on('click', function () {
        var type = $(this).data('type'); // 会返回当前点击元素的data-type属性的值, 此处返回 reload
        active[type] ? active[type].call(this) : '';  // 相当于call(reload)
    });
